...
# Endring i grenseprodukltiviteten
...
# Endring_mp = Z*a*(a-1)*x^(a-2)
  ...
  #Plot grenseproduktivitet for Z=2, a=.7 - blå linje
  
plotFun(mp(x,Z=2,a=.7)~x, xlim=range(0,10),
          xlab=list(label="Arbeidskraft", cex=1.2),
          ylab=list(label="MP", cex=1.2), main= "Grenseproduktivitet",
          ylim=c(0,5), col="blue") 
...
plotFun(ap(x,Z=2,a=.7)~x, xlim=range(0,10),
        xlab=list(label="Arbeidskraft", cex=1.2),
        ylab=list(label="AP", cex=1.2), main= "Gjennomsnittsproduktivitet",
        ylim=c(0,5), col="red") +
...
plotFun(mp(x,Z=2,a=.7)~x, xlim=range(0,10),
        xlab=list(label="Arbeidskraft", cex=1.2),
        ylab=list(label="MP, AP", cex=1.2), main= "MP (blå), AP (rød)",
        ylim=c(0,5), col="blue")


plotFun(ap(x,Z=2,a=.7)~x, xlim=range(0,10),
        xlab=list(label="Arbeidskraft", cex=1.2),
        ylab=list(label="MP, AP", cex=1.2), 
        ylim=c(0,5), col="red", add=TRUE)
